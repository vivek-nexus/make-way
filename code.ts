...
function validateAndSendState(headless: boolean): boolean {
  ...
  if (selection.length < 1) {
    headless
      ? figma.notify("Please select a top level node and rerun", {
        error: true,
        timeout: 5000,
        onDequeue: () => {
          figma.closePlugin()
  ...
  if (selection.length > 1) {
    headless
      ? figma.notify("Please select exactly one top level node and rerun", {
        error: true,
        timeout: 5000,
        onDequeue: () => {
          figma.closePlugin()
      ...
      : figma.ui.postMessage({ type: 'selectionState', state: 'INVALID', message: "Please select exactly one node" })
    return false
  }
...
function executeMovement(SPACE_TO_CREATE: number): void {
  ...
  try {
    ...
    figma.notify(`Pushing nodes on the right...`, { timeout: 2000 })
...
function PropagateShift(StartNode: SceneNode, Parent: ValidParentNode, ShiftAmount: number): number {
  if (!('children' in Parent)) return 0 // Return 0 if no shift can occur (default for safety)

  const startAbsBox = getAbsoluteBoundingBox(StartNode)
  if (!startAbsBox) return 0
